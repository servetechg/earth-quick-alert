/**
 * Fetch U.S. volunteer coordination center listings via Google Places API (New) Text Search.
 *
 * Usage:
 *   GOOGLE_MAPS_API_KEY=... npx tsx scripts/fetch-us-volunteer-centers.ts --limit 2
 *   GOOGLE_MAPS_API_KEY=... npx tsx scripts/fetch-us-volunteer-centers.ts --states AL,AK
 *   GOOGLE_MAPS_API_KEY=... npx tsx scripts/fetch-us-volunteer-centers.ts --continue
 *
 * Output: data/us-volunteer-coordination-centers.json (single file, append-friendly)
 */
...
const OUTPUT_PATH = path.join(process.cwd(), 'data', 'us-volunteer-coordination-centers.json');
const PAGE_SIZE = 20;
...
const SEARCH_QUERIES: { category: string; template: string }[] = [
    {
        category: 'volunteer_coordination_center',
        template: 'Volunteer Coordination Center in {stateName}',
    },
    { category: 'volunteer_center', template: 'Volunteer Center in {stateName}' },
    { category: 'disaster_volunteer_center', template: 'Disaster Volunteer Center in {stateName}' },
];

const FIELD_MASK =
    'places.id,places.displayName,places.formattedAddress,places.location,nextPageToken';

type VolunteerCenterRecord = {
    placeId: string;
    displayName: string;
...
type OutputFile = {
    ...
    volunteerCenters: VolunteerCenterRecord[];
};
...
function isDuplicate(record: VolunteerCenterRecord, dedup: DedupState): boolean {
...
function registerRecord(record: VolunteerCenterRecord, dedup: DedupState): void {
...
function buildDedupStateFromRecords(records: VolunteerCenterRecord[]): DedupState {
...
async function searchTextQueryAllPages(
    ...
    records: VolunteerCenterRecord[];
    pagesFetched: number;
    duplicatesSkipped: number;
    errors: ApiErrorLog[];
}> {
    const records: VolunteerCenterRecord[] = [];
...
async function searchVolunteerCentersForState(
    apiKey: string,
    stateCode: string,
    stateName: string,
    dedup: DedupState,
): Promise<{ records: VolunteerCenterRecord[]; summary: StateFetchSummary; errors: ApiErrorLog[] }> {
    const records: VolunteerCenterRecord[] = [];
    const errors: ApiErrorLog[] = [];
    ...
    for (const queryDef of SEARCH_QUERIES) {
        ...
        console.log(`[volunteer-fetch]   query: ${textQuery}`);
...
async function main() {
    ...
    const dedup = buildDedupStateFromRecords(output.volunteerCenters);
    ...
    if (statesToProcess.length === 0) {
        console.log('[volunteer-fetch] No states to process.');
        return;
    }

    console.log(
        `[volunteer-fetch] Processing ${statesToProcess.length} state(s): ${statesToProcess.map((s) => s.code).join(', ')}`,
    );
    ...
    for (const state of statesToProcess) {
        console.log(`[volunteer-fetch] Fetching ${state.name} (${state.code})...`);

        const { records, summary, errors } = await searchVolunteerCentersForState(
            apiKey,
            state.code,
            state.name,
            dedup,
        ...
        output.volunteerCenters.push(...records);
        ...
        console.log(
            `[volunteer-fetch] ${state.code}: retrieved=${summary.centersRetrieved} queries=${summary.queriesRun} pages=${summary.pagesFetched} duplicatesSkipped=${summary.duplicatesSkipped} errors=${errors.length}`,
        );
        console.log(`[volunteer-fetch] ${state.code} by category:`, summary.byCategory);

        if (errors.length > 0) {
            for (const err of errors) {
                console.error(
                    `[volunteer-fetch] ERROR ${state.code} [${err.searchCategory}] page ${err.page}: ${err.message}`,
                );
            }
        }
    }
    ...
    output.metadata.totalRecords = output.volunteerCenters.length;
    ...
    console.log('\n=== Volunteer center fetch summary ===');
