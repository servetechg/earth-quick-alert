/**
 * Fetch U.S. IT infrastructure locations via Google Places API (New) Text Search.
 *
 * Usage:
 *   GOOGLE_MAPS_API_KEY=... npx tsx scripts/fetch-us-it-infrastructure.ts --limit 2
 *   GOOGLE_MAPS_API_KEY=... npx tsx scripts/fetch-us-it-infrastructure.ts --states AL,AK
 *   GOOGLE_MAPS_API_KEY=... npx tsx scripts/fetch-us-it-infrastructure.ts --continue
 *
 * Output: data/us-it-infrastructure.json (single file, append-friendly)
 */
...
const OUTPUT_PATH = path.join(process.cwd(), 'data', 'us-it-infrastructure.json');
const PAGE_SIZE = 20;
...
const SEARCH_QUERIES: { category: string; template: string }[] = [
    {
        category: 'information_technology_center',
        template: 'Information Technology Center in {stateName}',
    },
    { category: 'government_data_center', template: 'Government Data Center in {stateName}' },
    {
        category: 'technology_operations_center',
        template: 'Technology Operations Center in {stateName}',
    },
];
...
type OutputFile = {
    ...
    itInfrastructureLocations: ItInfrastructureRecord[];
};
...
function isDuplicate(record: ItInfrastructureRecord, dedup: DedupState): boolean {
...
function registerRecord(record: ItInfrastructureRecord, dedup: DedupState): void {
...
function buildDedupStateFromRecords(records: ItInfrastructureRecord[]): DedupState {
...
async function searchTextQueryAllPages(
    ...
    records: ItInfrastructureRecord[];
    pagesFetched: number;
    duplicatesSkipped: number;
    errors: ApiErrorLog[];
}> {
    const records: ItInfrastructureRecord[] = [];
...
async function searchItInfrastructureForState(
    apiKey: string,
    stateCode: string,
    stateName: string,
    dedup: DedupState,
): Promise<{ records: ItInfrastructureRecord[]; summary: StateFetchSummary; errors: ApiErrorLog[] }> {
    const records: ItInfrastructureRecord[] = [];
    const errors: ApiErrorLog[] = [];
    ...
    for (const queryDef of SEARCH_QUERIES) {
        ...
        console.log(`[it-fetch]   query: ${textQuery}`);
...
async function main() {
    ...
    const dedup = buildDedupStateFromRecords(output.itInfrastructureLocations);
    ...
    if (statesToProcess.length === 0) {
        console.log('[it-fetch] No states to process.');
        return;
    }

    console.log(
        `[it-fetch] Processing ${statesToProcess.length} state(s): ${statesToProcess.map((s) => s.code).join(', ')}`,
    );
    ...
    for (const state of statesToProcess) {
        console.log(`[it-fetch] Fetching ${state.name} (${state.code})...`);

        const { records, summary, errors } = await searchItInfrastructureForState(
            apiKey,
            state.code,
            state.name,
            dedup,
        ...
        output.itInfrastructureLocations.push(...records);
        ...
        console.log(
            `[it-fetch] ${state.code}: retrieved=${summary.locationsRetrieved} queries=${summary.queriesRun} pages=${summary.pagesFetched} duplicatesSkipped=${summary.duplicatesSkipped} errors=${errors.length}`,
        );
        console.log(`[it-fetch] ${state.code} by category:`, summary.byCategory);

        if (errors.length > 0) {
            for (const err of errors) {
                console.error(
                    `[it-fetch] ERROR ${state.code} [${err.searchCategory}] page ${err.page}: ${err.message}`,
                );
            }
        }
    }
    ...
    output.metadata.totalRecords = output.itInfrastructureLocations.length;
    ...
    const errorsThisRun = runSummaries.reduce(
        (sum, s) => sum + output.metadata.errors.filter((e) => e.state === s.state).length,
        0,
    );

    console.log('\n=== IT infrastructure fetch summary ===');
    for (const s of runSummaries) {
        console.log(
            `  ${s.state} (${s.stateName}): ${s.locationsRetrieved} locations (${s.queriesRun} queries, ${s.pagesFetched} API pages)`,
        );
    }
...
main().catch((err) => {
    console.error('[it-fetch] failed:', err);
    process.exit(1);
});
