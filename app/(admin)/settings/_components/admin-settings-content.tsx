...
function readProfileSeedFromStorage(): ProfileSettings {
  if (typeof window === 'undefined') return INITIAL_PROFILE
  const name = localStorage.getItem('userName')?.trim() ?? ''
  const email = localStorage.getItem('userEmail')?.trim() ?? ''
  const profilePic = localStorage.getItem('userProfilePic')?.trim() ?? ''
  return {
    ...INITIAL_PROFILE,
    ...(name ? { name } : {}),
    ...(email ? { email } : {}),
...
export function AdminSettingsContent({ variant }: { variant: AdminSettingsVariant }) {
  const router = useRouter()
  const pathname = usePathname()
  const photoInputRef = useRef<HTMLInputElement>(null)
  const canConfigureSessionTimeout = variant === 'super'
  ...
  useLayoutEffect(() => {
    ...
    if (variant === 'super') {
      if (role === 'sub-admin') {
        router.replace('/sub-admin-settings')
        return
      }
      if (role !== 'super-admin') {
    ...
    const seeded = readProfileSeedFromStorage()
    const hasProfileSeed = Boolean(seeded.name || seeded.email || seeded.profilePic)
    setProfile(seeded)
    setProfileLoading(!hasProfileSeed)

    setIsAuthorized(true)

    const raw = new URLSearchParams(window.location.search).get('tab')
    if (raw && isSettingsTabKey(raw)) {
      setActiveTab(raw)
    }
  }, [variant, router, pathname])
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      const [profileData, notifData, dispatchPayload, securityPayload] = await Promise.all([
      ...
      try {
        if (!profileRes.ok) {
          throw new Error(
            typeof profileData.error === 'string' ? profileData.error : 'Failed to load profile',
          )
        }
      ...
      try {
        if (!notifRes.ok) {
          throw new Error(
            typeof notifData.error === 'string'
              ? notifData.error
              : 'Failed to load notification preferences',
          )
        }
        if (notifData.success && notifData.data?.notificationPreferences) {
          setNotifications(
            normalizeNotificationPreferences(notifData.data.notificationPreferences as Record<string, unknown>),
          )
        }
      } catch (e: unknown) {
        if (!cancelled) {
      ...
      try {
        if (securityRes.ok && securityPayload?.success && securityPayload?.data && !cancelled) {
          setIsTwoFactorEnabled(Boolean(securityPayload.data.twoFactorEnabled))
          setIsSessionTimeoutEnabled(securityPayload.data.sessionTimeoutEnabled !== false)
        }
      } catch {
        /* ignore */
      }
  ...
  const handleSave = async (section: string) => {
    ...
    if (section === 'Security') {
      ...
      try {
        ...
        const securityBody: Record<string, boolean> = { twoFactorEnabled: isTwoFactorEnabled }
        if (canConfigureSessionTimeout) {
          securityBody.sessionTimeoutEnabled = isSessionTimeoutEnabled
        }
