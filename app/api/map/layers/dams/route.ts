...
import { queryDamsByBounds, queryDamsByState } from '@/lib/gis/layers/nid-dams-query';
...
/**
 * GET /api/map/layers/dams?state=TX
 * GET /api/map/layers/dams?state=TX&west=…&south=…&east=…&north=…
 */
export async function GET(req: Request) {
    try {
        ...
        if (bounds) {
            const { markers, cached } = await queryDamsByBounds(bounds, {
        ...
        const { markers, cached } = await queryDamsByState(stateKey!, { force });
        return NextResponse.json({
            markers,
            count: markers.length,
            stateKey,
        ...
        console.error('map/layers/dams GET:', error);
        return NextResponse.json({ error: 'Failed to fetch dams layer' }, { status: 500 });
    }
}
