...
import {
    getPoliceDeploymentForUser,
    mergePoliceDeploymentForUser,
    type PoliceDeploymentPayload,
...
export async function GET() {
    ...
    const payload = await getPoliceDeploymentForUser(g.session.user.id, licenseId, rf);
...
export async function PUT(req: NextRequest) {
    ...
    const licenseId = g.session.user.licenseId ?? null;
    const rf = String(g.session.user.responderFunction || '');
    const next = await mergePoliceDeploymentForUser(g.session.user.id, licenseId, body, rf);
