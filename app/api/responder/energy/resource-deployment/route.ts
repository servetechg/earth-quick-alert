...
export async function GET() {
    ...
    const payload = await getEnergyResourceDeploymentForUser(g.session.user.id, licenseId, rf);
...
export async function PUT(req: NextRequest) {
    ...
    const licenseId = g.session.user.licenseId ?? null;
    const rf = String(g.session.user.responderFunction || '');
    const next = await mergeEnergyResourceDeploymentForUser(g.session.user.id, licenseId, body, rf);
