...
export async function GET() {
    ...
    return NextResponse.json(getNationalGuardResourceDeployment());
}
...
export async function PUT(req: NextRequest) {
    ...
    const cur = getNationalGuardResourceDeployment();

    const sites = Array.isArray(body.sites)
        ? body.sites.map((entry: unknown, i: number) => {
              const o = entry as Record<string, unknown>;
              const st = o.status === 'limited' || o.status === 'suspended' ? o.status : 'active';
              return {
                  id: typeof o.id === 'string' ? o.id : `ng-${i}`,
    ...
    const next: NationalGuardResourceDeploymentPayload = {
        ...cur,
    ...
    setNationalGuardResourceDeployment(next);
