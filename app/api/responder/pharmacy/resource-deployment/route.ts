...
export async function GET() {
    ...
    const payload = await getPharmacyResourceDeploymentForUser(g.session.user.id, licenseId, rf);
...
export async function PUT(req: NextRequest) {
    ...
    const licenseId = g.session.user.licenseId ?? null;
    const rf = String(g.session.user.responderFunction || '');
    const next = await mergePharmacyResourceDeploymentForUser(g.session.user.id, licenseId, body, rf);
