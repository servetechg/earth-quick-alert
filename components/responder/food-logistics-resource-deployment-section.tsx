...
import { Loader2, Save, MapPin, Users, Plus, Trash2, Edit, Package } from 'lucide-react'
...
function statusBadge(status: FoodLogisticsSiteStatus) {
  switch (status) {
    case 'active':
...
export function FoodLogisticsResourceDeploymentSection({ compact }: Props) {
  const [data, setData] = useState<FoodLogisticsResourceDeploymentPayload | null>(null)
  ...
  const [fStatus, setFStatus] = useState<FoodLogisticsSiteStatus>('active')
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const res = await fetch('/api/responder/food-logistics/resource-deployment')
      if (!res.ok) {
  ...
  const persist = async (next: FoodLogisticsResourceDeploymentPayload) => {
    setSaving(true)
    try {
      const res = await fetch('/api/responder/food-logistics/resource-deployment', {
        method: 'PUT',
      ...
      setData(await res.json())
      toast.success('Food & supply logistics updated (mock)')
      return true
    } catch (e: unknown) {
  ...
  if (loading) {
    return (
      ...
      >
        ...
        Loading food & supply logistics deployment…
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-black tracking-tight text-slate-900">Food & Supply Logistics — Dashboard Response Network</h2>
          <p className="text-sm text-slate-500 mt-1">Volunteers, distribution sites, and supply chain management</p>
        </div>
      </div>
      ...
      <Card className={RESPONDER_PANEL_CARD}>
        ...
        <CardContent className="space-y-8 px-0">
          ...
          <p className="text-sm text-slate-600">
            Each row is a distribution point, warehouse, or volunteer staging area with <strong>volunteers deployed</strong> at
            that location.
          </p>
          ...
          <div className="space-y-3">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
              <h4 className="text-sm font-black uppercase tracking-widest text-slate-500">Response network locations</h4>
              <Button
                type="button"
                size="sm"
                  ...
                  {data.sites.length === 0 ? (
                    <tr>
                      <td colSpan={5} className="px-4 py-10 text-center text-slate-500">
                        No locations yet. Click <strong>&quot;Add Detail&quot;</strong> to add a distribution or volunteer site.
                      </td>
                    </tr>
                  ) : (
                    data.sites.map((r, i) => (
        ...
        <DialogContent className="border-slate-200 bg-white text-slate-900 sm:max-w-lg">
          <DialogHeader>
            <DialogTitle className="font-black text-lg tracking-tight text-slate-900">
              {siteDialogMode === 'create' ? 'Add distribution / volunteer site' : 'Edit distribution / volunteer site'}
            </DialogTitle>
            <DialogDescription className="text-xs text-slate-500">
              Set volunteers deployed at this location; coordinates drive GIS markers.
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-3 py-1">
            ...
            <div className="space-y-1.5">
              ...
              <Select value={fStatus} onValueChange={(v) => setFStatus(v as FoodLogisticsSiteStatus)}>
