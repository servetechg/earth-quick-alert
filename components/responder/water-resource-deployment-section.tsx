...
import { Loader2, Save, MapPin, Droplets, Plus, Trash2, Edit, HardHat } from 'lucide-react'
...
function statusBadge(status: WaterCrewStatus) {
  switch (status) {
    case 'active':
...
export function WaterResourceDeploymentSection({ compact }: Props) {
  const [data, setData] = useState<WaterResourceDeploymentPayload | null>(null)
  ...
  const [fStatus, setFStatus] = useState<WaterCrewStatus>('active')
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const res = await fetch('/api/responder/water/resource-deployment')
      if (!res.ok) {
  ...
  const persist = async (next: WaterResourceDeploymentPayload) => {
    setSaving(true)
    try {
      const res = await fetch('/api/responder/water/resource-deployment', {
        method: 'PUT',
      ...
      setData(await res.json())
      toast.success('Water deployment updated (mock)')
      return true
    } catch (e: unknown) {
  ...
  if (loading) {
    return (
      ...
      >
        ...
        Loading water company deployment…
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-black tracking-tight text-slate-900">Water Company — Resource Deployment</h2>
          <p className="text-sm text-slate-500 mt-1">Water crews and resource deployment management</p>
        </div>
      </div>
      ...
      <Card className={RESPONDER_PANEL_CARD}>
        ...
        <CardContent className="space-y-8 px-0">
          ...
          <p className="text-sm text-slate-600">
            Each row is a water crew staging area or repair point with <strong>crews deployed</strong> at
            that location.
          </p>
          ...
          <div className="space-y-3">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
              <h4 className="text-sm font-black uppercase tracking-widest text-slate-500">Water crew locations</h4>
              <Button
                type="button"
                size="sm"
                  ...
                  {data.sites.length === 0 ? (
                    <tr>
                      <td colSpan={5} className="px-4 py-10 text-center text-slate-500">
                        No locations yet. Click <strong>&quot;Add Detail&quot;</strong> to add a water crew deployment site.
                      </td>
                    </tr>
                  ) : (
                    data.sites.map((r, i) => (
                      <tr key={r.id} className="hover:bg-cyan-50/30 transition-colors">
                        ...
                        <td className="px-4 py-4 tabular-nums font-semibold text-cyan-700">{r.crewsDeployed}</td>
        ...
        <DialogContent className="border-slate-200 bg-white text-slate-900 sm:max-w-lg">
          <DialogHeader>
            <DialogTitle className="font-black text-lg tracking-tight text-slate-900">
              {siteDialogMode === 'create' ? 'Add water crew location' : 'Edit water crew location'}
            </DialogTitle>
            <DialogDescription className="text-xs text-slate-500">
              Set water crews deployed at this location; coordinates drive GIS markers.
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-3 py-1">
            ...
            <div className="space-y-1.5">
              ...
              <Select value={fStatus} onValueChange={(v) => setFStatus(v as WaterCrewStatus)}>
